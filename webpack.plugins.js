...
const templateParameters = require('./src/frontend/config/template-parameters');
const routeTemplates = require('./src/frontend/config/route-templates');
        ...
        (acc, [, config]) => {
            if (config.output) {
                acc.push(new HtmlWebpackPlugin({
                    template: `src/frontend/templates/${config.template}`,
                    inject: false,
